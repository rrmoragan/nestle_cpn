...
if( !defined('PROJECT_GRNO') ){
	...
	class Proyecto_Grano{
		...
		public function pack($code=''){
			...
			$a = $a[0];
			$id = $a['entity_id'];
			log_data($this->log,'id product pack ==> '.$id.' sku ==> '.$a['sku']);
			...
			$pmqn = '';
			if( isset( $_POST['umaquina'] ) ){ $pmqn = 'MCH-'.$_POST['umaquina']; }

			$s = $this->sql_pack( $id, $pmqn );
			$b = query( $s );
			if( $b!=null ){
				foreach ($b as $et => $r) {
		...
		public function product_id( $id_prod=0 ){
			...
			$b['qty'] = $this->product_id_stock( $id_prod );
			if($b['qty']==null){ return null; }
		...
		public function product_id_stock($id_product=0){
			if( $id_product==0 ){ 
				log_data($this->log,'product_id_stock(null)');
				return null; }

			$s = $this->sql_product_id_stock( $id_product );
			$a = query( $s );
			if($a==null){
				log_data($this->log,'qty ==> null');
				return null;
			}
			...
			//log_data( $this->log,'qty ==> '.print_r($a,true) );
			return $a;
		}
		...
		private function sql_pack($pack_id=0,$mq=''){
			if( $pack_id==0 ){ return ''; }
		...
		private function sql_product_id( $id_prod=0 ){
			...
			$this->sql_log( 'sql_product_id( '.$id_prod.' )' );
			return $s;
		}
		...
		private function sql_product_id_stock($id_product=0){
			...
			$s = "SELECT * FROM cataloginventory_stock_item where product_id=$id_product and is_in_stock=1";
			$this->sql_log( $s );
			return $s;
		}
		...
		public function is_venta_proces_d($email=''){
			if($email==''){ return 0; }

			$s = $this->sql_is_venta_proces_d($email);
			$a = query($s);
			if($a==null){ return 0; }
		...
		private function sql_is_venta_proces_d( $email='' ){
			if($email==''){ return ''; }

			$s = "SELECT distinct(sfoi.order_id), sfo.customer_email, sfo.created_at,sfo.state,sfo.status,sfo.increment_id
			from sales_flat_order_item as sfoi
			inner join sales_flat_order as sfo on sfo.entity_id=sfoi.order_id
			where sfoi.sku IN(
				'7613035194960','7613034293381','7613032864491','7613032870652','7613033024290','7501059273269','7613031587377','7613034609250','7613032396350','7613031794331','7501059273245','7501059273689','7613031526406','7501059273252','7613036272049'
			) 
			and sfo.customer_email like '$email'
			order by sfoi.order_id DESC
			";
			return $s;
		}

		public function is_machine_order_id( $sku_machine='' ){
			log_data($this->log,'is_machine_order_id()');
			log_data($this->log,'machine ==> '.$sku_machine);

			if( $sku_machine=='' ){ return null; }

			$s = $this->sql_is_machine_order_id($sku_machine);
			$a = query( $s );
			if($a==null){ return null; }
		...
		private function sql_is_machine_order_id($machine_sku=''){
			if($machine_sku==''){ return ''; }
