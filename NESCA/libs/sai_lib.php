...
class Sai_lib{
	...
	public function carga($v=0){
		...
		if( $this->select_sales_order( $this->data['sales_order'], $v ) ){
	...
	public function select_sales_order( $sales_order='',$v=0 ){
		if( $v ){ echo "\n select_sales_order()"; }

		/* obteniendo datos de la orden de venta */
			if( $v ){ echo "\n select_sales_order() ==> paso 1"; }

			$a = $this->select_salles_order_data( $sales_order, $v );
			if( $a==null ){
				if( $v ){ echo "\n select_sales_order() => null"; }
				return false;
			}
	...
	private function select_salles_order_data( $sales_order='', $v=0 ){
		...
		$s = "SELECT
			...
			where 
			sfoa.parent_id = ( SELECT entity_id from sales_flat_order where increment_id like '$sales_order' )
		...
		if( $b==null ){ 
			...
			echo "\n todas las direcciones deben llevar:";
			echo "\n\t calle, delegacion y codigo postal ademas de existir una direccion de envio\n";
			return null;
		}
		if( $b==null ){
