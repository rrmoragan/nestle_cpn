...
class report{
	...
	/*******************************************/
	public function report_null(){
		$s = "TRUNCATE rep_ventas";
		query($s);
		$s = "TRUNCATE rep_ventas_items";
		query($s);
	}
	...
	public function select_all_sales(){
		...
		$this->update_rep_ventas( $b );

		$this->data['sales'] = $b;
		return count( $b );
	}

	private function update_rep_ventas($d=null){
		if($d==null){ return false; }

		foreach ($d as $et => $r) {
			$s = '';

			if( $r['customer_id']=='' ){ $r['customer_id']=0; }
			...
			$s = $r['entity_id'].
				...
				','."'".$r['increment_id']."'".
				','."'".$r['customer_email']."'".
				','."'".$r['order_currency_code']."'".
				','.$r['subtotal'].
				','.$r['subtotal_incl_tax'].
				','.$r['shipping_amount'].
				','.$r['shipping_tax_amount'].
				','."'".$r['shipping_method']."'".
				','.$r['discount_amount'].
				','.$r['tax_amount'].
				','.$r['grand_total'].
			...
			$cols = "`entity_id`,`status`,`customer_id`,`billing_address_id`,`shipping_address_id`,`sales_order`,`customer_email`,`order_currency_code`,`subtotal`,`subtotal_incl_tax`,`shipping_amount`,`shipping_tax_amount`,`shipping_method`,`discount_amount`,`tax_amount`,`grand_total`,`total_item_count`,`total_qty_ordered`,`created_at`,`updated_at`,`remote_ip`,`x_forwarded_for`,`peso_volumetrico`";
			$s = "INSERT INTO rep_ventas( $cols ) values( $s )";
			tt($s);
		}
