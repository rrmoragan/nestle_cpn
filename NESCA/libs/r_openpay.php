...
if( !defined('PG_VER') ){
	...
	class openpay_mlg{
		...
		// lista todas las ordenes de venta con los registros openpay
		// v1.5
		public function list_sales_order( $sort='' ){
			...
			$s = "SELECT
				...
				sfo.customer_email,
				ce.entity_id as customer_id,
				( select value from customer_entity_varchar as cev inner join eav_attribute as ea1 on ea1.attribute_id = cev.attribute_id where ea1.attribute_code like 'openpay_user_id' and cev.entity_id = ce.entity_id ) as openpay_user_id,
				( select sfop.openpay_payment_id from sales_flat_order_payment as sfop where sfop.parent_id = sfo.entity_id ) as openpay_payment_id,
				( select ov.status from openpay_validate as ov where ov.sales_order = sfo.increment_id ) as openpay_status,
				sfomt.dinamic,
				...
				from sales_flat_order as sfo
				left join customer_entity           as ce    on ce.email = sfo.customer_email
				left join sales_flat_order_mlg_type as sfomt on sfomt.sales_order_id = sfo.entity_id
				left join sales_flat_order_address  as sfoad on sfoad.parent_id = sfo.entity_id
				left join customer_blocked as blocked on blocked.cemail = sfo.customer_email
				where
				sfoad.address_type like 'shipping'
				and
				( blocked.id IS NULL OR sfo.status like 'pagado' )
				;";
			$a = query($s);
			if($a==null){
				return 0;
