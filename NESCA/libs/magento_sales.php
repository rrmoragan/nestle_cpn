...
if( !defined( 'M_SALES' ) ){
	...
	define('M_SALES','Magento Sales 3.3');
	define('M_SALES_UPDATE','2019-09-25');
	...
	class mSales{
		...
		*/
		public function list_sales( $order = 'ASC', $limit=0, $status = '' ){
			// v1.3

			$st = '';
			if( $status!='' ){ $st = " where sfo.status like '$status' "; }
			...
			if( $order == 'ASC' || $order == 'DESC' ){
				$order = " order by sfo.increment_id $order ";
			}else{
				$order = " order by sfo.increment_id ASC ";
			}

			$blocked = new mCustomer();

			$s = "SELECT sfo.* from sales_flat_order as sfo $st $order $limit ";
			//echo "\n sql ==> $s";
			$a = query($s);
			if( $a==null ){
				$this->data = null;
			...
			$b = null;
			foreach ($a as $et => $r) {
				if( isset( $r['created_at'] ) ){ $a = explode(' ', $r['created_at']); $r['created_at']=$a[0]; }
				if( isset( $r['updated_at'] ) ){ $a = explode(' ', $r['updated_at']); $r['updated_at']=$a[0]; }
				$b[ $r['increment_id'] ] = $r;
			}
		...
		// lista todos los items disponibles en base de datos
			public function list_sales_items(){
				$this->data = null;
				$s = "SELECT 
					count( sfoi.sku ) as nsku,
					sfo.increment_id,
					sfo.status
		...
		public function sales_update( $so='',$campo='', $data=null ){
			...
			switch ( $campo ) {
				...
				case 'cfdi':
					...
					$this->sales_update_address_billing( $so, 'uso_cfdi', $data );
					return true;
					break;
				case 'rz':
		...
		public function list_campos_items( $tipo='' ){
			$a = array(
				...
				'varchar' => array(
					'name', 'nombre_secundario', 'codigo_sap', 'sat_clave', 'sat_descrip', 'sat_unidad', 'sat_clave_unidad', 'configurable_max_prods', 'am_shipping_type', 'sku_alterno','margen'
				),
				'gral' => array(
		...
		public function sales_order_items( $so=null, $v=false ){
			...
			/* validando todos los campos */
				...
				foreach ($b as $et => $r) {
					...
					//echo "\n codigo_barras ==> ".$r['sku'];
					//echo "\n margen_descuento ==> ".$r['margen_descuento'];
					//echo "\n margen ==> ".$r['margen'];

					$margen = 0;
					if( isset( $r['margen_descuento'] ) ){
						$margen = $r['margen_descuento'];
					}

					if( isset( $r['margen'] ) && ( $r['margen'] != $r['margen_descuento'] ) ){
	...
	class reportVentas{
		...
		/* functiones reporte de ventas facturacion */
		/* obtiene el registro del reporte correspondiente a la orden de ventas indicada */
		...
		/* obtiene los items relacionados con una orden de ventas del reporte de ventas */
			public function rv_sales_items(){
				if( $this->data == null ){ return 0; }

				$id = $this->data['rs_id'];
				$s = "SELECT * from report_sales_items where rs_id=$id";
				$a = query( $s );
				if( $a==null ){ return 0; }
		...
		/* lista todas las ordenes de venta con registros de facturacion */
			public function rv_list_sales_in1(){
				$this->data = null;
				$s = "SELECT * from report_sales where factura_xml IS NOT NULL OR factura_xml <> ' ' ";
				$a = query( $s );
		...
		/* agrega 1 registro de orden de venta al reporte */
			public function rv_add( $dat=null, $v=false ){
				...
				/* agregando registro principal */
					echo "\n ... actualizando report_sales";
					$s = "INSERT into report_sales ( $campos ) values( $valores )";
					$this->sql = $s;

					$id = query( $s );
					if( $id==null ){ echo " ... [error]"; return null; }

				/* agregando items */
					echo "\n ... actualizando report_sales_items";
					//echo "\n sales_order ==> [".$dat['increment_id']."]";
					$n = $this->rv_add_items( $id, $dat, $v );
					if( $n==0 ){ echo " ... [error]"; return 0; }
		...
		/* elimina todos las ordenes de ventas del reporte contenidas en el arreglo */
			...
			public function rv_list( $order='ASC', $limit=0 ){
				if( $limit>0 ){ $limit = " limit 0,$limit "; }else{
					$limit = '';
				}
				$s = "SELECT * from report_sales order by sales_order $order $limit";
				$a = query($s);
				if( $a==null ){
					$this->data = null;
					return 0;
		...
		// elimina los registros duplicados en report_sales
			public function debug_table_report_sales(){

				// obteniendo todos los registros
					$s = "SELECT rs.rs_id, rs.entity_id, rs.sales_order, rs.customer_email, rs.sales_order, rs.status, rs.updated_at  from report_sales as rs order by sales_order DESC, updated_at DESC;";
					$a = query( $s );
					if( $a==null ){ return 0; }
				...
				// obteniendo duplicados
					$b = null;
					foreach ($a as $et => $r) {
						if( !isset( $b[ $r['sales_order'] ] ) ){
							$b[ $r['sales_order'] ] = array(
								'n' => 0,
								'data' => ''
							);
						}
						$b[ $r['sales_order'] ]['data'] .= '::'.$r['rs_id'];
						$b[ $r['sales_order'] ]['n']++;
					}

				// quitando elementos con 1 unidad
					foreach ($b as $et => $r) {
						if( $r['n'] == 1 ){
							unset( $b[ $et ] );
						}
				...
				// borrando registros duplicados
					...
					foreach ($b as $et => $r) {
						foreach ($r as $etr => $rr) {
							$s = "DELETE from report_sales where rs_id = ".$rr;
							query( $s );
							$i++;
						}
					}
		...
		/* lista las diferencias entre los registros */
			public function debug_diferences(){
				$s = "SELECT
					rs.rs_id,
					rs.sales_order,
					rs.status,
					rs.total_item_count,
					rs.total_qty_ordered,
					rs.updated_at,
					( select count(*) from report_sales_items as rsi where rsi.rs_id = rs.rs_id ) as so_qty,
					( rs.total_item_count - ( select count(*) from report_sales_items as rsi where rsi.rs_id = rs.rs_id ) ) as diff

					from report_sales as rs
					order by sales_order ASC";
				$a = query( $s );
				return $a;
			}
		/* realiza un revicion en el reporte de ventas */
			public function rv_verify(){
				// PASO 1
					$n = $this->debug_table_report_sales();
					echo "\n registros duplicados en tabla report_sales = ".$n;

				/* obteniendo sumatorias */
					$a = $this->debug_diferences();
					if( $a==null ){
						echo "\n verify ==> null";
						return 0;
					}

				/* dejando solo los registros con sumatorias diferentes de cero */
					foreach ($a as $et => $r) {
						if( $r['diff'] == 0 ){ unset( $a[ $et ] ); }
					}

					if( $a==null ){
						echo "\n todos los registros cuadran";
						return 0;
					}
					...
					$bloqued = $this->list_sales_no_valid();
					if( $bloqued!=null ){
						foreach ($b as $et => $r) {
							if( isset( $bloqued[ $r ] ) ){ unset( $b[ $et ] ); }
						}
					}

				/* marcando datos a recalcular */
					if( $b == null ){
						return 0;
					}
		...
		/* lista los sales order que no seran validados */
			public function list_sales_no_valid(){
				$s = "SELECT * from report_sales_valid";
				$s = "SELECT 
					rsv.*,
					rs.status
					from report_sales_valid as rsv
					inner join report_sales as rs on rs.sales_order = rsv.sales_order
					order by status ASC";
				$a = query( $s );
				if( $a==null ){
					return null;
		...
		/* agrega un sales order para ser validado */
			public function add_sales_no_valid( $so='' ){
				if( $so=='' ){
					echo "\n sin sales order";
					return 0;
				}
				...
				$s = "INSERT into report_sales_valid( sales_order,updated_at ) values( '$so','$date' )";
				//echo "\n sql ==> $s";
				$id = query( $s );
				if( $id==null ){ return 0; }
		...
		/* quita un sales order para ser validado */
			public function remove_sales_no_valid( $sov=0 ){
				if( $sov == 0 ){ return false; }

				$s = "DELETE from report_sales_valid where rsv_id = $sov ";
				//echo "\n sql ==> $s";
				query( $s );
				return true;
			}
		...
		/* modifica el cosot de envio par el reporte de ventas */
			public function shipping_recalc( $so='',$val=0 ){
				if( $so=='' ){ return false; }
				...
				$precio = $val;
				$precio_iva = $val * 0.16;
				$sum = $precio + $precio_iva;
				$mt = $this->data['margen_total'];
				...
				$this->rv_update( $id, 'shipping_table_rate', $precio );
				$this->rv_update( $id, 'shipping_op_comision', 0.00 );
				$this->rv_update( $id, 'shipping_op_transaccion', 0.00 );
				$this->rv_update( $id, 'shipping_calc_sum', $sum );
				$this->rv_update( $id, 'shipping_description', 'cotizado por MLG $'.$val.' + iva' );
				$this->rv_update( $id, 'margen_sobrante', $margen );
		...
		/* modifica el cosot de envio par el reporte de ventas */
			public function margen_update( $so='',$val=0 ){
				if( $so=='' ){ return false; }

				$this->rv_sales( $so );
				$this->rv_update( $this->data['rs_id'], 'margen_total', $val );
...
select a.sales_order, a.items, a.total_items, (a.items - a.total_items) as diff
from ( 

	select 
	"100001009" as sales_order,
	count(*) as items,
	( select total_item_count from report_sales where sales_order like '100001009' ) as total_items

	from report_sales_items 
	where rs_id = ( select rs_id from report_sales where sales_order like '100001009' )
...
========================================================
	...
	CREATE TABLE `report_sales_shipping` (
		`rss_id` INT NOT NULL AUTO_INCREMENT,
		`rs_id` INT NOT NULL,
		`shipping` varchar(255) NOT NULL,
		`shipping_descrip` varchar(255) NOT NULL,
		`nivel` INT NOT NULL,
		`calc` INT NOT NULL,
		PRIMARY KEY (`rs_id`)
	) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='shipping descripcion relacion';
	...
	CREATE TABLE `report_sales_valid` (
	...
	ALTER TABLE `report_sales_valid` ADD PRIMARY KEY (`rsv_id`);
	ALTER TABLE `report_sales_valid` MODIFY `rsv_id` int(11) NOT NULL AUTO_INCREMENT;
