...
class Sai_lib{
	...
	public function carga(){
		...
		if( $this->select_salles_order( $this->data['sales_order'] ) ){
	...
	public function set_sales_order( $data='' ){
		if( $data=='' ){ return false; }
		$this->data['sales_order'] = $data;
		return true;
	}
	public function set_program( $data='' ){
		if( $data=='' ){ return false; }
		$this->data['program'] = $data;
		return true;
	}
	public function set_envio( $data=0 ){
		if( $data==0 ){ return false; }
		$this->data['envio'] = $data;
		return true;
	}
	public function set_canje( $data=0 ){
		if( $data==0 ){ return false; }
	...
	public function select_salles_order( $sales_order='' ){

		/* obteniendo datos de la orden de venta */
			$a = $this->select_salles_order_data( $sales_order );
		...
		/* obteniendo direccion de envio y completando datos */
			$s = $this->sql_user_address_data( $this->user_address_id( $b[0]['customer_id'] ) );
			$a = query( $s );
			if( $a == null ){
			...
			foreach ($b as $et => $r) {
				...
				if( $c['telephone_extension'] != 'NULL' ){ $b[ $et ]['telefono_b'] = $c['telephone_extension']; }
	...
	public function user_address_id( $customer_id='' ){
	...
	private function select_salles_order_data( $sales_order='' ){
