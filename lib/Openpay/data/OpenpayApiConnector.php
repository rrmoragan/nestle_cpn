...
class OpenpayApiConnector
{
    ...
    private function _curlRequest($method, $absUrl, $headers, $params, $auth = null) {
        ...
        $s = "\n method ==> ".print_r( $method,true ).
             "\n absUrl ==> ".print_r( $absUrl,true ).
             "\n headers ==> ".print_r( $headers,true ).
             "\n params ==> ".print_r( $params,true ).
             "\n auth ==> ".print_r( $auth,true );

	Mage::log("$s", Zend_Log::INFO, 'x_openpay.log');

        OpenpayConsole::trace('OpenpayApiConnector @_curlRequest');

	Mage::log("oconsole ???", Zend_Log::INFO, 'x_openpay.log');
        ...
        $s = "\n rbody ==> ".print_r( $rbody,true ).
             "\n rcode ==> ".print_r( $rcode,true );
    ...
    private function encodeToJson($arr) {
        $s = "\n encodeToJson ==> arr ==> ".print_r( $arr,true );	Mage::log("$s", Zend_Log::INFO, 'x_openpay.log');

        $encoded = json_encode($arr);
        $s = "\n encodeToJson ==> encoded ==> ".print_r( $encoded,true );	Mage::log("$s", Zend_Log::INFO, 'x_openpay.log');

        if (!function_exists('mb_detect_encoding')) {
                $s = "\n mb_detect_encoding ==> function mb_detect_encoding no detect";
                Mage::log("$s", Zend_Log::INFO, 'x_openpay.log');
        }

        $s = "\n encodeToJson ==> mb_detect_encoding ==> ".
		print_r( mb_detect_encoding($encoded),true );	Mage::log("$s", Zend_Log::INFO, 'x_openpay.log');
