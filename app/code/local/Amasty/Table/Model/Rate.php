...
class Amasty_Table_Model_Rate extends Mage_Core_Model_Abstract
{
    ...
    public function findBy($request, $collection)
    {
        ...
        foreach ($ratesTypes as $key => $value){
            ...
            foreach ($intersectTypes[$key] as $shippingType){
                ...
                Mage::log( '=====> sum subtotal ==> '.print_r( $totals,true ), Zend_Log::INFO, 'envios.log');
                $comisiones[] = $totals;
    ...
    public function openpay_comision(){
        ...
        $openpay = json_decode( $openpay,true );
        if( $openpay == null ){ return null; }
    ...
    public function calc_comision($dat=null, $subtotal_all=null, $products=null){
        ...
        // dat
            //      contiene el subtotal del envio
            //    [n] => Array
            //    (
            //        [cost] => 210
            //        [time] =>
        ...
        // subtotal
            //      contiene el listado de todos los productos
            //          not_free_price ==> precio del producto sin impuestos
            //          not_free_weight ==> peso en kg del producto
            //          qty ==> cantidad
            //          tax_amount ==> impuesto aplica al producto por el numero de piezas
            //          discount_amount ==> descuento aplicado al producto
            //      mas un registro adicional con la sumatoria del los productos
        ...
        // obtiene las comixiones openpay en 1 sola variable
            // {"transaccion":2.5,"comicion":0.029}
            $op = $this->openpay_comision();
            //Mage::log( " comiciones openpay ==> ".print_r( $op,true ), Zend_Log::INFO, 'envios.log');
        // lprods ==> contiene Id_producto y cantidad por producto
            // (
            //     [200] => Array (
            //             [getEntityId] => 200
            //             [qty] => 2
        ...
        // en caso de no haber registro de productos abortar calculo customizado
            if( !$products ){ return $dat; }
        // recorre todos los productos y obtiene el margen
        ...
        // margen total
            ...
            foreach ($lprods as $et => $r) {
                $margen_total = $margen_total + $r['margen_sum'];
            }

            //Mage::log( 'margen_total ==> '.print_r($margen_total,true), Zend_Log::INFO, 'envios.log');
        // envio
            $envio = 0;
            $envio_indx = 0;
            foreach ($dat as $et => $r) {
            ...
            //Mage::log( 'envio ==> '.print_r($envio,true), Zend_Log::INFO, 'envios.log');
        // subtotal
            $subtotal = 0;
            ...
            if( $subtotal == 0 ){
                ...
                //$ss = '';
                //foreach ($quote as $et => $r) {
                //    $ss .= "\n\t -- $et";
                //}
                //Mage::log( $ss, Zend_Log::INFO, 'envios.log');
                //Mage::log( $quote['grand_total'], Zend_Log::INFO, 'envios.log');
                $subtotal = $quote['grand_total'];
            }

            //Mage::log( 'subtotal ==> '.print_r($subtotal,true), Zend_Log::INFO, 'envios.log');
        // registro log
            $ss = "\n\t".'cantidad pedida ==> '.json_encode( $this->_prev_data );
            Mage::log( $ss, Zend_Log::INFO, 'envios.log');

            $lc = null;
            $lc[] = "margen = $margen_total";
            $lc[] = "comision_porcent = ".$op['comicion'];
            $lc[] = "transaccion = ".$op['transaccion'];
            $lc[] = "envio = $envio";
            $lc[] = "subtotal = $subtotal";

            $original_data = array(
                'margen' => $margen_total,
                'comision' => $op['comicion'],
                'transaccion' => $op['transaccion'],
                'envio' => $envio,
                'subtotal' => $subtotal,
            );
        ...
        // calculo
            // PASO 1
                $lc[] = "envio = envio - margen";
                $lc[] = "envio = $envio - $margen_total";

                $envio = $envio - $margen_total;
                if( $envio>0 ){
                    $margen_total = 0;
                }else{
                    $margen_total = $envio * -1;
                    $envio = 0;
                }
            ...
            // PASO 2
                $lc[] = "comision = subtotal + envio";
                $lc[] = "comision = $subtotal + $envio";

                $t = $subtotal + $envio;

                $lc[] = "comision = ( comision * ".( $op['comicion'] * 100 )."% ) + transaccion";
                $lc[] = "comision = ( $t * ".$op['comicion']." ) + ".$op['transaccion'];

                $comision = ( $op['comicion'] * $t ) + $op['transaccion'];
                $lc[] = "comision = $comision";

            // PASO 3
                $lc[] = "envio = envio + comision";
                $lc[] = "envio = $envio + $comision";
                $envio += $comision;
                $lc[] = "envio = $envio";
            ...
            // PASO 4
                ...
                $original_data['envio_total'] = $envio;
                $original_data['margen_total'] = $margen_total;

                $lc[] = "envio = $envio";
                $lc[] = "margen = $margen_total";

                $ss = "\n\t".'data ==> '.print_r( $original_data,true );
                Mage::log( $ss, Zend_Log::INFO, 'envios.log');
            ...
            Mage::getSingleton('core/session')->setEnvioData($original_data);
            Mage::getSingleton('core/session')->setEnvioCantidadPedida($this->_prev_data);
            Mage::getSingleton('core/session')->setEnvioCalculo($lc);

            Mage::log( " ===> $envio", Zend_Log::INFO, 'envios.log');
            $dat[ $envio_indx ]['cost'] = $envio;
