            ...
            foreach ($args as $key => $arg) {
                if (($arg instanceof Mage_Core_Model_Layout_Element)) {
                    if (isset($arg['helper'])) {
                        ...
                        if( $helperMethod == 'cart' ){
                            $nn = count($arg);
                            if($nn==1 && $arg[0]==null ){ $arg=null; }
                        }
                        
                        Mage::log( Mage::helper($helperName), Zend_Log::INFO, 'system.log');
                        Mage::log( $helperMethod, Zend_Log::INFO, 'system.log');
                        Mage::log( $arg, Zend_Log::INFO, 'system.log');
