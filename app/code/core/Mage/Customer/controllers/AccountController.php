...
class Mage_Customer_AccountController extends Mage_Core_Controller_Front_Action
{
            ...
            switch ($configAddressType) {
                case Mage_Customer_Model_Address_Abstract::TYPE_SHIPPING:
                    $userPrompt = $this->__('If you are a registered VAT customer, please click <a href="%s">here</a> to enter you shipping address for proper VAT calculation',
                        $this->_getUrl('customer/address/new/is/billing'));
    ...
    /* modif rmorales@mlg.com.mx */
    private function forgotpassword_log( $email='' ){
        if($email==''){ return false; }

        $t = time()-21700;
        $date = date( 'Y-m-d', $t );
        $time = $date." ".date('G:i', $t );
        $ip = $_SERVER['REMOTE_ADDR'];
        if(isset( $_SERVER['HTTP_X_REAL_IP'] )){
            $ip = $_SERVER['HTTP_X_REAL_IP'];
        }

        $s = "ip[".$ip."][".$time."] recuperacion de password email ==> [$email]";

        Mage::log( $s , Zend_Log::INFO, 'forgotpassword--'.$date.'.log');
        return true;
    }
    /* fin modif */
}
