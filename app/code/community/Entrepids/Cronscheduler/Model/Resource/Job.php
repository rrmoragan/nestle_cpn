...
*/
class Entrepids_Cronscheduler_Model_Resource_Job extends Mage_Core_Model_Resource_Db_Abstract
{
    /** @var bool */
    ...
    /**
     * @param Entrepids_Cronscheduler_Model_Job $object
     * @param mixed                   $value
     * @param null                    $field
     *
     * @return $this
    ...
    {
        if (!$object instanceof Entrepids_Cronscheduler_Model_Job) {
            throw new InvalidArgumentException(sprintf("Expected object of type 'Entrepids_Cronscheduler_Model_Job' got '%s'", get_class($object)));
        }

        /** @var Entrepids_Cronscheduler_Model_Job $object */

        if (!empty($field)) {
            throw new InvalidArgumentException('Entrepids_Cronscheduler_Model_Resource_Job cannot load by any field except the job code.');
        }
    ...
    public function getJobDataFromModel(Entrepids_Cronscheduler_Model_Job $job)
    {
        $values = array(
            'name'                      => $job->getName(),
            ...
            'visible'                   => ($job->getVisible() ? '1' : '0'),
            'event_notification'        => $job->getEventNotification(),
            'email_cron_notification'   => $job->getEmailCronNotification()
        );
    ...
    public function setModelFromJobData(Entrepids_Cronscheduler_Model_Job $job, array $data)
    {
        ...
        $job->setVisible(isset($data['visible']) ? $data['visible'] : '');
        $job->setEventNotification(isset($data['event_notification']) ? $data['event_notification'] : 0);
        $job->setEmailCronNotification(isset($data['email_cron_notification']) ? $data['email_cron_notification'] : '');
        return $job;
    }
}
