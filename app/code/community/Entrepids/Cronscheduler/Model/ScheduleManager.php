...
*/
class Entrepids_Cronscheduler_Model_ScheduleManager{ // extends Aoe_Scheduler_Model_ScheduleManager{
    ...
    public function generateSchedules()
    {
        ...
        $jobs = Mage::getSingleton('cronscheduler/job')->getCollection();
     ...
     * @return $this
     */
    public function generateSchedulesForJob(Entrepids_Cronscheduler_Model_Job $job)
    {
    ...
    /**
     * Clean up the history of tasks, save data in ht_cron_schedule
    ...
    public function cleanup(){
        ...
        $logFile = Mage::getStoreConfig('system/cron/logFile');
        ...
        foreach ($history->getIterator() as $record) { /* @var $record Aoe_Scheduler_Model_Schedule */
            if (isset($historyLifetimes[$record->getStatus()])) {
                if(strtotime($record->getExecutedAt()) < $now - $historyLifetimes[$record->getStatus()]) {
                    $data = $record->getData();
                    if(isset($data['executed_at']) && !empty($data['executed_at'])){
                        unset($data['schedule_id']);
                        Mage::getModel('cronscheduler/htcron')->setData($data)->save();
                    }
                    $record->delete();
                }
            }
        ...
        if ($logFile) {
            $duration = microtime(true) - $startTime;
            Mage::log('History backup saved (Duration: ' . round($duration, 2) . ' sec)', null, $logFile);
        }
