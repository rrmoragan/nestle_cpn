 ...
 */
class Entrepids_Cronscheduler_Model_Schedule extends Mage_Cron_Model_Schedule{
    ...
    /**
     * @var Entrepids_Cronscheduler_Model_Job
     */
    protected $job;
     ...
     * @return $this
     */
    public function initializeFromJob(Entrepids_Cronscheduler_Model_Job $job)
    {
    ...
    public function runNow($tryLockJob = true, $forceRun = false)
    {
        ...
        try {
            ...
            if ((is_string($messages) && strtoupper(substr($messages, 0, 6)) == 'ERROR:') || $this->getStatus() === Aoe_Scheduler_Model_Schedule::STATUS_ERROR) {
                ...
                Mage::helper('cronscheduler/data')->sendErrorMail($this, $messages);
                $msg = 'ERROR: Ocurrió un error durante el proceso.';
                Mage::helper('cronscheduler/data')->sendNotificationMail($this, Entrepids_Cronscheduler_Helper_Data::SEND_NOTIFICATION_ON_ERROR,$msg);
                ...
                $msg = 'El proceso no concluyó y se programó para ejecutarse de nuevo.';
                Mage::helper('cronscheduler/data')->sendNotificationMail($this, Entrepids_Cronscheduler_Helper_Data::SEND_NOTIFICATION_ALWAYS,$msg);
                ...
                $msg = 'CORRECTO: El proceso se ejecutó correctamente.';
                Mage::helper('cronscheduler/data')->sendNotificationMail($this, Entrepids_Cronscheduler_Helper_Data::SEND_NOTIFICATION_ON_SUCCESS,$msg);
            ...
            $this->setStatus(Aoe_Scheduler_Model_Schedule::STATUS_ERROR);
            $errormsg = 'ERROR: Ocurrió un error durante la ejecución del proceso. ' . $e->__toString();
            Mage::helper('cronscheduler/data')->sendNotificationMail($this, Entrepids_Cronscheduler_Helper_Data::SEND_NOTIFICATION_ON_ERROR,$errormsg);
    ...
    {
        if (is_null($this->job)) {
            $this->job = Mage::getModel('cronscheduler/job')->load($this->getJobCode());
        }
        return $this->job;
    }
    ...
    public function markAsDisappeared($message = null)
    {
        ...
        $this
            ...
            ->save();
        $errormsg = 'ERROR: El proceso se perdió durante la ejecución y no se concretó la tarea.';
        Mage::helper('cronscheduler/data')->sendNotificationMail($this, Entrepids_Cronscheduler_Helper_Data::SEND_NOTIFICATION_ON_ERROR,$errormsg);
    ...
    public function kill()
    {
        ...
        $this
            ...
            ->save();
        $errormsg = 'ERROR: El proceso excedio el tiempo de ejecución y tuvo que ser terminado.';
        Mage::helper('cronscheduler/data')->sendNotificationMail($this, Entrepids_Cronscheduler_Helper_Data::SEND_NOTIFICATION_ON_ERROR,$errormsg);
