 ...
 */
class Entrepids_Cronscheduler_Block_Adminhtml_Timeline extends Mage_Adminhtml_Block_Widget_Container
{
    ...
    {
        $this->filter = Mage::app()->getRequest()->getParam('job_code');
    ...
    protected function _prepareLayout()
    {
        ...
        $this->_addButton('filter', array(
            'label' => $this->__('Filter'),
            'onclick' => "filterJobcode()",
        ));
    ...
    protected function loadSchedules()
    {
        ...
        foreach ($collection as $schedule) {
            $jc = $schedule->getJobCode(); //jobCode
            $_jobData = Mage::getModel('cronscheduler/job')->load($jc); 
            $jobName = strtolower($_jobData->getName());
            $filter = strtolower($this->getFilter());
            if(in_array($_jobData->getVisible(),array('false','0',0))){
                continue;
            }
            if(!empty($filter) &&  strpos($jc,$filter)===FALSE && strpos($jobName,$filter)===FALSE  ){
                continue;
            }
            /* @var Entrepids_Cronscheduler_Model_Schedule $schedule */
            ...
            $this->schedules[$jc][] = $schedule;
            $this->alias[$jc] = $_jobData->getName();
        }
    ...
    /**
     * Get alias for job codes
     *
     * @return array
     */
    public function getAliasJobCodes()
    {
        return $this->alias;
    }
     ...
     * @return string
     */
    public function getGanttDivAttributes(Entrepids_Cronscheduler_Model_Schedule $schedule)
    {

        if ($schedule->getStatus() == Entrepids_Cronscheduler_Model_Schedule::STATUS_RUNNING) {
