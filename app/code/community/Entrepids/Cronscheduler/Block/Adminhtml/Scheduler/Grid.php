...
*/
class Entrepids_Cronscheduler_Block_Adminhtml_Scheduler_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
    /**
    ...
    protected function _prepareColumns()
    {
        ...
        switch (Mage::getStoreConfig('system/cron/listCodeFilterType')) {
            case Aoe_Scheduler_Model_Adminhtml_System_Config_Source_List_Code_Filtertype::SELECT:
                $config['type']    = 'options';
                $config['options'] = Mage::getSingleton('cronscheduler/job')->getCollection()->toOptionHash('job_code', 'name');
                break;
     ...
     * @return string
     */
    public function decorateMessages($value, Entrepids_Cronscheduler_Model_Schedule $row)
    {
        $return = '';
        if (!empty($value)) {
